...
mycursor.execute("create table if not exists bank_master(acno char(15) primary key,name varchar(30),city char(20),mobileno char(10),balance int(100))")
mycursor.execute("create table if not exists banktrans(acno char (15),amount int(6),dot date,ttype char(1),foreign key (acno) references bank_master(acno))")
mydb.commit()
while(True):
    print("-"*60)
